...
#include <WiFi.h> // Essential library for Wi-Fi connectivity
#include <WiFiClientSecure.h> // Library for secure Wi-Fi communications, necessary for the Telegram bot
...
// Variable to keep track of the last Telegram update ID
long lastUpdateID = 0; // Initialize to 0 to fetch all new messages on startup
...
void connectToWiFi() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        // Set the Telegram certificate for secure connection.
        // This certificate is embedded in the UniversalTelegramBot library for ESP boards.
...
void startWatering(unsigned long durationMs, String source) {
    if (!isWateringActive) {    // Only start if the pump is not already active
        ...
        wateringStartTime = millis();   // Record the start time of watering
    ...
    } else { // This 'else' refers to the if (!isWateringActive)
...
void stopWatering(String reason) {
    if (isWateringActive) {
        ...
        isWateringActive = false; // Set the flag to indicate watering is no longer active
...
void handleNewMessages(int numNewMessages, unsigned long currentMillis) {
    ...
    for (int i = 0; i < numNewMessages; i++) {
        ...
        // Update the lastUpdateID to ensure we only get new messages
        ...
        if (text == "/start" || text == "/menu") {
            // Added empty string for parse_mode as the 4th argument
            ...
            if (isnan(temperature) || isnan(humidity)) {
                ...
                if (isWateringActive) {
                    // Use currentMillis to calculate remaining watering time
                    unsigned long durationToUse = (wateringStatusMessage.indexOf("Manual") != -1 ? MANUAL_WATERING_DURATION_MS : AUTOMATIC_WATERING_DURATION_MS);
            ...
            // Added empty string for parse_mode as the 4th argument
            bot.sendMessageWithReplyKeyboard(chat_id, "I don't understand that command. Please use the menu options.", keyboardJson, "", true, true);
        }
    }
}
...
void loop() {
    ...
    // --- Handle incoming Telegram messages ---
    // Use the global lastUpdateID to fetch new messages
